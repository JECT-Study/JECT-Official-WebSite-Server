...
class AdminMailDispatchSecurityTest extends ApplicationPeriodTest {
    ...
    @Test
    @AuthenticatedUser(isAdmin = false)
    @DisplayName("일반 사용자는 발송 작업 목록을 조회할 수 없다")
    void 일반_사용자는_발송_작업_목록을_조회할_수_없다() throws Exception {
        mockMvc.perform(get("/admin/mails/dispatches"))
                .andExpect(status().isUnauthorized());
    }
    ...
    void 다른_관리자는_발송_작업_상세를_조회할_수_없다() throws Exception {
        ...
        willThrow(new MailException(MailErrorCode.DISPATCH_JOB_NOT_FOUND))
                .given(mailDispatchQueryService).getJob(51L, 100L);
    ...
    @Test
    @AuthenticatedUser(isAdmin = true, memberId = 51L)
    @DisplayName("다른 관리자는 발송 작업의 수신자 결과를 조회할 수 없다")
    void 다른_관리자는_발송_작업의_수신자_결과를_조회할_수_없다() throws Exception {
        // given
        willThrow(new MailException(MailErrorCode.DISPATCH_JOB_NOT_FOUND))
                .given(mailDispatchQueryService)
                .searchTargets(eq(51L), eq(100L), eq(null), any());
